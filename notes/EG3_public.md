...
In-paper anchor: `paper/BIRATIONAL_CLASSIFICATION_PREPRINT.md` (`BCL_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of admissible birational reductions, flips, fibrations, and endpoint models through a multi-lane birational-classification super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`BCL_G3` closes when `kappa_compact` provides the compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_BCL`.
...
If near-failure extraction and no-Zeno spacing hold, then `BCL_G3` prevents hidden drift before rigidity is applied.
