...
In-paper anchor: `paper/BIRATIONAL_CLASSIFICATION_PREPRINT.md` (bridge and final strict margin).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of admissible birational reductions, flips, fibrations, and endpoint models through a multi-lane birational-classification super-architecture`.
...
2. the endpoint-transfer lock `birational_lock` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_BCL` remains positive.
...
Payload: verify `birational_lock` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_BCL` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `birational_lock`
